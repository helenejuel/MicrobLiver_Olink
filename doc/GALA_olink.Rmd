...
sign_cytokines <- anova_GALA_kleiner %>%
    filter(Adjusted_pval < 0.05) %>% # Bonferroni = 92 tests, however adjusted pval already adjusted with Benjamini-Hochberg
    pull(Assay)
...
sign_cytokines <- lm_GALA_te %>%
    filter(Adjusted_pval < 0.05) %>% # Bonferroni = 92, however adjusted pval already adjusted with Benjamini-Hochberg
    pull(Assay)
...
lm_GALA_death <- olink_lmer(GALA, 
           variable = "", 
           outcome = "NPX",
           random = "cohort")

# Extract list of significant cytokines
sign_cytokines <- lm_GALA_death %>%
    filter(Adjusted_pval < 0.05) %>% # adjusted pval already adjusted with Benjamini-Hochberg
    pull(Assay)
print(sign_cytokines) #  cytokines significant
```
